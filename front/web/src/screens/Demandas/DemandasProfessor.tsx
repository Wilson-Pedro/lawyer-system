    ...
    const fetchDemandas = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/demandas/professor/${professorId}?page=${page}&size=${size}`,
          {
            headers: {
              ...
              <tbody>
                {filteredDemandas.map((demanda) => (
                  <tr key={demanda.id}>
                    ...
                    <td
                      className={getStatusClass(demanda.demandaStatusProfessor)}
                    >
                      {demanda.demandaStatusProfessor.toLocaleLowerCase() ===
                      'null'
                        ? 'Aguardando...'
                        : demanda.demandaStatusProfessor}
                    </td>
                    <td
                      className={getStatusClass(demanda.demandaStatusAdvogado)}
                    >
                      {demanda.demandaStatusAdvogado.toLocaleLowerCase() ===
                      'null'
                        ? 'Aguardando...'
                    ...
                    <td className="text-center">
                      <button
                        onClick={() =>
                          navigate(
                            `/demandas/professor/${professorId}/demanda/${demanda.id}/editar`,
                          )
                        }
                    ...
                    <td className="text-center">
                      <button
                        onClick={() =>
                          navigate(`/demandas/${demanda.id}/respostas`)
                        }
