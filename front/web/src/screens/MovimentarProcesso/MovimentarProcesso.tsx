...
export default function MovimentarProcesso() {
    ...
    const buscarProcessos = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/processos/buscar/${numeroDoProcessoSearch}?page=${page}&size=${size}`,
          {
            headers: {
  ...
  useEffect(() => {
    const movimentos = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/movimentos/buscar/${numeroDoProcesso}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <div className="d-flex align-items-center justify-content-between mb-4">
          <div className="d-flex align-items-center gap-3">
            <h2 className="fw-bold text-dark mb-0">
              Movimentos do Processo {numeroDoProcesso || ''}
            </h2>
          </div>
          ...
          <Button
            ...
            onClick={() =>
              navigate(`/processos/${numeroDoProcesso}/movimento/cadastrar`)
            }
            disabled={disabled}
          >
